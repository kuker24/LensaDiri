...
- **Hobby release baseline:** `main` @ **`c535a6b`** | tag **`v1.0.0-hobby-final`**. Product freeze was explicitly reopened on 2026-07-26 for a **frontend-only total redesign**. Backend/domain scope remains frozen. Full PRD **stops ~75%** (honest; not 100%).
- **Active scope:** approved frontend UI/UX redesign plus reactive security, deps, health, monitoring, retention, backup readiness, and regression tests. Frontend PRs stay small, CI green, rollback clear, health 200.
- **Hard ban:** product/backend features beyond the approved redesign, paid residual services, AI narrative ON, WCAG/psychometric cert claims, publish modules without reviewers, Full Spectrum over-cap publish.
...
- Next.js 16.2.12 App Router dan React 19.2.7.
...
- PRD 2.0 adalah canonical contract. Composer + Complex + provisional precision production ON (2026-07-26); AI OFF. Full Spectrum tetap draft sampai capacity/product gate. Status hanya naik dengan evidence.
- Existing `quick` dan `standard` legacy tetap valid; modular label Quick/Normal/Complex. deep_self_discovery pilot; full_spectrum draft.
...
- Admin read-only DB-backed (`/admin/*`) ada; mutasi admin, formal production/accessibility verification, dan psychometric validation belum selesai. Recovery Resend transport + optional login gate ada di kode tetapi default OFF (tanpa secret / flag); production delivery dan mandatory verification tetap butuh approval eksplisit.
...
- **Deploy wajib production.** Production di-deploy lewat CLI, bukan lewat Git. `git push` hanya menghasilkan Preview dan tidak pernah memindahkan alias production, jadi setiap kali diminta deploy (atau diminta push lalu deploy) wajib jalankan `vercel --prod --yes`. "Pushed" bukan "deployed"; jangan laporkan perubahan sudah live hanya karena sudah di-push.
- Verifikasi deploy pada alias live sebelum melapor live. Health 200 tidak cukup karena build lama juga 200; buktikan sesuatu yang baru dari commit itu, misalnya path aset baru balik 200 dengan ukuran byte sama seperti file lokal.
- Preview build memang gagal (`Server environment configuration is invalid.` saat prerender `/login`) karena Preview tidak punya server env values dan guard-nya fail closed. Preview merah itu wajar, bukan bukti commit rusak. Jangan melonggarkan env guard dan jangan menyalin nilai Production ke Preview; Preview butuh database terisolasi plus secret sendiri.
- CI GitHub tidak menjaga branch redesign: trigger hanya `main`, `agent/**`, dan PR ke `main`. Selama kerja di `design/total-redesign`, gate lokal urutan CI adalah satu-satunya pengaman, jadi jalankan lengkap sebelum deploy.
