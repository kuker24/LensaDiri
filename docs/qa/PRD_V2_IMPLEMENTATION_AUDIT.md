...
| Live email dan mandatory verification       | §23.1   | `IMPLEMENTED` dormant  | Resend transport + `FEATURE_REQUIRE_EMAIL_VERIFICATION` di kode; default OFF tanpa secret. Production tetap dormant sampai approval + `RESEND_API_KEY`/`EMAIL_FROM`. Login gate opsional; akun lama `email_verified_at` null tetap lolos sampai flag ON.                                                     |
