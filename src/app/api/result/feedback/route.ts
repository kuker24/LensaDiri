...
import { resultFeedbackSchema } from "@/lib/validation/assessment";
import { apiFailure, apiSuccess, getDatabaseFailureStatus, noStoreHeaders } from "@/server/http";
import { createResultFeedback } from "@/server/repositories/assessment";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const parsed = await parseJsonRequest(request, resultFeedbackSchema);
  if (!parsed.success)
  ...
  try {
    const limited = await consumeRateLimit(
      resultHash,
      assessmentRateLimitPolicies.resultFeedback,
    ...
    const created = await createResultFeedback({
      message: parsed.data.message || null,
      rating: parsed.data.rating,
      resultTokenHash: resultHash,
    });
    return NextResponse.json(
      created ? apiSuccess({ status: "received" }) : apiFailure("not_found"),
      { headers: noStoreHeaders, status: created ? 201 : 404 },
    );
  } catch (error) {
