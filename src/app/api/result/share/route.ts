...
import { createResultShare } from "@/server/repositories/assessment";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const created = await createResultShare({
      expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1_000),
      resultTokenHash: resultHash,
      shareTokenHash: hashOpaqueToken(shareToken, environment.tokenHashPepper),
    });
    return NextResponse.json(created ? apiSuccess({ shareToken }) : apiFailure("not_found"), {
      headers: noStoreHeaders,
      status: created ? 201 : 404,
    });
  } catch (error) {
