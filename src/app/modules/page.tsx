    ...
    <main className="container-shell py-12 sm:py-18">
      ...
      <ul className="mt-10 grid gap-5 md:grid-cols-2" aria-label="Daftar modul LensaDiri">
        {modules.map((module) => {
          ...
          return (
            <li className="border-line rounded-lg border bg-white p-6" key={module.key}>
              ...
              {!available && module.availabilityReason ? (
                <p className="border-aperture-soft bg-aperture-soft text-ink mt-4 rounded-md border p-4 text-sm leading-6">
                  {module.availabilityReason}
                </p>
              ) : null}
              <Link
                className="focus-ring border-line text-ink hover:border-lens hover:text-lens mt-5 inline-flex min-h-11 items-center rounded-sm border px-4 font-semibold transition-colors duration-150 ease-out"
