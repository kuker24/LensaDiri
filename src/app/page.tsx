    ...
    <>
      <section className="container-shell grid gap-12 pt-16 pb-20 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:pt-24 lg:pb-28">
        <div>
          ...
          <h1 className="font-display max-w-3xl text-5xl font-semibold tracking-[-0.045em] text-balance sm:text-6xl lg:text-7xl">
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            <Link
              className="focus-ring bg-lens text-canvas hover:bg-lens-strong rounded-md px-6 py-4 text-center font-semibold shadow-[0_10px_15px_-3px_rgb(139_92_246_/_0.2),0_4px_6px_-4px_rgb(139_92_246_/_0.2)] transition-colors duration-150 ease-out"
              href="/start"
            >
              Mulai eksplorasi
            </Link>
            <Link
              className="focus-ring border-line text-ink hover:border-lens/30 rounded-md border bg-white/70 px-6 py-4 text-center font-semibold transition-[color,background-color,border-color] duration-150 ease-out hover:bg-white"
              href="/method"
            >
              Pelajari metode
            </Link>
        ...
        <div className="lens-glow border-line relative overflow-hidden rounded-lg border bg-white/82 p-6 backdrop-blur-[18px] sm:p-8">
          <div className="relative">
              ...
              {scores.map(([label, value]) => (
                <div key={String(label)}>
                  ...
                  <div className="bg-line h-1.5 overflow-hidden rounded-full">
                    <div
                      aria-hidden="true"
                      className="h-full rounded-full bg-linear-to-r from-violet-500 to-teal-400"
                      style={{ width: `${value}%` }}
                    />
                  </div>
                </div>
            ...
            <p className="border-lens-soft bg-lens-soft/60 text-lens-strong mt-8 rounded-md border p-4 text-sm leading-6">
      ...
      <section
        className="border-line border-y bg-white/45 py-20 backdrop-blur-[18px]"
        id="cara-kerja"
      >
        <div className="container-shell">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {steps.map(([number, title, description]) => (
              <article className="border-line rounded-lg border bg-white/82 p-6" key={number}>
      ...
      <section className="container-shell py-20 lg:py-28">
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
          ...
          <Link
            className="focus-ring text-lens hover:text-lens-strong inline-flex min-h-11 items-center rounded-md font-semibold"
            href="/method"
          >
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
          {lenses.map((lens) => (
            <article
              className="border-line rounded-lg border bg-white/82 p-7 backdrop-blur-[18px]"
              key={lens.title}
            >
      ...
      <section className="container-shell pb-20 lg:pb-28">
        <div className="bg-ink text-canvas overflow-hidden rounded-lg px-6 py-10 sm:px-10 lg:grid lg:grid-cols-[0.85fr_1.15fr] lg:gap-14 lg:px-14 lg:py-14">
          <div>
            <p className="text-aperture-on-dark text-sm font-semibold tracking-[0.16em] uppercase">
              Trust by design
            </p>
            <h2 className="font-display mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">
