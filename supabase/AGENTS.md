...
- `cleanup_expired_retention_data(timestamptz)` deletes only expired guest sessions, rate-limit buckets older than 90 days, and audit security events older than 365 days; it is idempotent and never touches account-owned results. `preview_expired_retention_data(timestamptz)` is its read-only dry-run companion that only counts eligible rows. Audit append-only trigger allows DELETE only when `lensadiri.account_hard_delete` or `lensadiri.retention_cleanup` is on for the transaction. Both functions are security-definer and revoked from `anon`/`authenticated`.
