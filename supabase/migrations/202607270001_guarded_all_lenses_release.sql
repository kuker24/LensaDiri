-- Additive guarded rollout for production databases where canonical item banks already exist.

-- Feature flags are intentionally untouched.
...
begin
  select count(*)::integer into target_module_count
  from public.modules
  where key in (
  ...
  select count(*)::integer into target_version_count
  from public.module_versions
  inner join public.modules on modules.id = module_versions.module_id
  where modules.key in (
    'three_center', 'instinct', 'socionics_communication',
    'riasec', 'attachment', 'psychosophy'
  ) and module_versions.version = '1.0.0';

  select count(*)::integer into target_question_count
  ...
  if target_module_count = 0 and target_version_count = 0 and target_question_count = 0 then
    return;
  end if;

  if target_module_count <> 6 or target_version_count <> 6 or target_question_count <> 147 then
    raise exception 'guarded all-lenses release requires canonical 6 modules, 6 versions, and 147 questions'
      using errcode = '55000';
  end if;
end;
$$;
